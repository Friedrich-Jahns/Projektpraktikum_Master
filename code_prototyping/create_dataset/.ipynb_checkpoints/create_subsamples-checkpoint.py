...
dat_path = cwd.parent.parent.parent / "data" 
path_list = [
   dat_path
   / "PE-2024-01126-M_00_s0021_PM_Complete_Transmittance_Stitched_Flat_v004.h5"
]
...
# print(paths[1])
paths = [data_files_path/'PE-2024-01126-M_00_s0021_PM_Complete_Transmittance_Stitched_Flat_v004.h5']
# key = input('Dat. Nr.')
...
for img in tqdm(paths):
    ...
    for i in tqdm(range(0, shape[0] - (res_size + shape[0] % res_size), res_size // 2)):
        for j in tqdm(
            ...
            plt.imsave(
                save_path / "img_uncol" / f"{img.stem}_{i}_{j}.png",
                sub_img.image_data(),
                cmap="gray",
            )
            plt.imsave(
                save_path / "mask_uncol" / f"{img.stem}_{i}_{j}.png",
