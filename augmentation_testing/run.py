...
def main():
    ...
    model = torch.compile(model)  # einmalig vor dem Training (Test)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), leave=False):
            t0 = time.time()
            images, masks = batch
            images, masks = images.to(device, memory_format=torch.channels_last), masks.to(device, memory_format=torch.channels_last)
            #optimizer.zero_grad()
            t1 = time.time()
            print(images.shape)
        ...
        # print(f"Datenladen: {load_time:.2f}s")
