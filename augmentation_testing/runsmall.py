...
def main():
    ...
    plt.plot(range(1, len(log_arr[1]) + 1), log_arr[1], label='train_loss_bce')
    plt.plot(range(1, len(log_arr[2]) + 1), log_arr[2], label='val_loss_dice')
    plt.plot(range(1, len(log_arr[3]) + 1), log_arr[3], label='val_loss_bce')
    plt.legend()
